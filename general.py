...
def get_energy(G,density_list,H_core,enr):
    offset = 0
    Eer_dimer = 0
...
def Heitler_london(guess_list,noc_list,S,super_mol,H_core,enr):
      p=orthogonal_density(guess_list,noc_list,S)
      J,K =scf.rhf.get_jk(super_mol,dm=(p))
      g=J-0.5*K
      HL_E=2*np.trace(H_core@p)+2*np.trace(np.dot(p,g))+enr
      return HL_E
    
def orthogonal_density(guess_list,noc_list,S):
      orbital=[]
...
def generalized_dimer(molecules,guess_C=None,only_HL_energy=False):
  ...
  G = get_G(super_mol,density)
  #print(G)

  e = get_energy(G,density,H_core,enr)
  print(noc_list)
  HL_E = Heitler_london(guess,noc_list,S,super_mol,H_core,enr)
  print(f"HL energy for {n} H2O cluster is {HL_E}.")
  if only_HL_energy:
      return HL_E
...
#molecules=mol()
generalized_dimer(molecules,only_HL_energy=True)
